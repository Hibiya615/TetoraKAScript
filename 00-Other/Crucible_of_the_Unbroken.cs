...
public class Crucible_of_the_Unbroken
{
    ...
    {
        Init, // 初始
        Behemoth, // 高二左路随机#5：贝希摩斯
        Lauda // 高二尾王：魔斧之主 劳妲
    }
    Beasts_Phase phase = Beasts_Phase.Init;
    
    [ScriptMethod(name: "高二左路随机#5：贝希摩斯", userControl: false, eventType: EventTypeEnum.AddCombatant, eventCondition: ["DataId:regex:^19731$"])]
    public void 贝希摩斯转换(Event @event, ScriptAccessory accessory)
    {
        if (HelperExtensions.GetCurrentTerritoryId() != 1343) return;
        phase = phase switch
        { 
            Beasts_Phase.Init => Beasts_Phase.Behemoth,
            _ => phase
        };
        if(isDeveloper) accessory.Method.SendChat($"/e [Debug]：战斗进度：随机#5：贝希摩斯");
    }
    ...
    public void 劳妲转换(Event @event, ScriptAccessory accessory)
    {
        ...
        { 
            ...
            Beasts_Phase.Behemoth => Beasts_Phase.Lauda,
            _ => phase
        };
    ...
    */
    ...
    [ScriptMethod(name: "随机#4: 奇子·莫古小术_绒绒陨石雨（步进扩大地火）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^5094[3-6]$"])]
    public void 莫古小术_绒绒陨石雨(Event @event, ScriptAccessory accessory)
    {
        ...
        dp1.Name = $"绒绒陨石雨步进预测{@event.SourceId}";
        dp1.Color = dp.Color = accessory.Data.DefaultDangerColor;
        dp1.Owner = dp.Owner = @event.SourceId();
        dp1.Offset = new Vector3(0f, 0f, -8f);
        dp1.DestoryAt = dp.DestoryAt = @event.ActionId== 50943 ? 6700 : 1600;
        switch (@event.ActionId())
        {
            case 50943:
                dp.Scale = new Vector2(6f); 
    ...
    {
        IntPtr omenHandle = accessory.Method.VfxMethod.CreateOmen(574, new Vector3(40f),
            @event.EffectPosition(), @event.SourceRotation(), new Vector4(1f,1f,0f,1f), 8700);
        /* 能用三角了就不画扇形了！
        var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "随机#4: 奇子·莫古小歌_莫古狂想曲（撞球提示）", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:regex:^49397$"])]
    public void 莫古小歌_莫古狂想曲(Event @event, ScriptAccessory accessory)
    {
        if (isTTS)accessory.Method.TTS($"避开BOSS撞球");
    }
    ...
    {
        if (isText)accessory.Method.TextInfo($"连续放置5个陨石，不要重叠", duration: 20800, true);
    ...
    public void 雷塔_放电(Event @event, ScriptAccessory accessory)
    {
        ...
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"{@event.SourceName()}放电连线{@event.SourceId}";
        dp.Owner = accessory.Data.Me;
        dp.Color = accessory.Data.DefaultSafeColor;
        dp.ScaleMode |= ScaleMode.YByDistance;
        dp.TargetObject = @event.SourceId();
    ...
    [ScriptMethod(name: "随机#5: 雷塔_放电销毁", eventType: EventTypeEnum.ObjectEffect, eventCondition: ["Id1:1","Id2:2"],userControl:false)]
    public void 雷塔_放电销毁(Event @event, ScriptAccessory accessory)
    {
        accessory.Method.RemoveDraw($".*放电连线.*");
    }
    ...
    [ScriptMethod(name: "随机#5: 奇子·贝希摩斯_大漩涡（钢铁）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^49415$"])]
    public void 贝希摩斯_大漩涡钢铁(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"贝希摩斯_大漩涡钢铁{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
        dp.Scale = new Vector2(12f);
        dp.DestoryAt = 5700;
    ...
    public void 贝希摩斯_大漩涡月环(Event @event, ScriptAccessory accessory)
    {
        if (isTTS)accessory.Method.TTS($"靠近BOSS");
        
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"贝希摩斯_大漩涡月环{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
        dp.Scale = new Vector2(40f);
        dp.InnerScale = new Vector2(8f);
    ...
    [ScriptMethod(name: "随机#5: 奇子·铁巨人_巨剑（顺劈）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^49408$"])]
    public void 铁巨人_巨剑(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"铁巨人_巨剑{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
        dp.Scale = new Vector2(30f);
        dp.Radian = 120f.DegToRad(); 
