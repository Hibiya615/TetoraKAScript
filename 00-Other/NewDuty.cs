...
[ScriptType(guid: "80890eac-4730-4708-ad1b-05aba469c2a1", name: "最新最热临时绘制", territorys: [1307, 1346],
    version: "0.0.1.3", author: "Tetora", note: noteStr)]
...
*/
    ...
    */
    ...
    [ScriptMethod(name: "—————— 两岐塔：魔之塔 ——————", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:"])]
    public void 两岐塔魔之塔(Event @event, ScriptAccessory accessory) { }

    [ScriptMethod(name: "BOSS1 双头怪鸟_决战（防火墙）", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:regex:^419[24]$"])]
    public void 双头怪鸟_决战(Event @event, ScriptAccessory accessory)
    {
        if (@event.TargetId() != accessory.Data.Me) return; 
        var DecisiveBattle = @event.StatusId == 4192 ? "绿头" : "蓝头";
        if (isText) accessory.Method.TextInfo($"攻击{DecisiveBattle}", duration: 4700, true);
        if (isEdgeTTS)accessory.Method.EdgeTTS($"攻击{DecisiveBattle}");
        if (isTTS) accessory.Method.EdgeTTS($"攻击{DecisiveBattle}");

    }
    
    [ScriptMethod(name: "BOSS1 蓝头_剧毒吐息（中心钢铁）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47617$"])]
    public void 普通蓝头_剧毒吐息(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"蓝头_剧毒吐息";
        dp.Color = accessory.Data.DefaultDangerColor;
    ...
    [ScriptMethod(name: "BOSS1 绿头_风暴吐息（中心击退）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^48243$"])]
    public void 普通绿头_风暴吐息(Event @event, ScriptAccessory accessory)
    {
    ...
    [ScriptMethod(name: "BOSS1 双头怪鸟_雷霜暴风雨 AOE", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47735$"])]
    public void 普通双头怪鸟_雷霜暴风雨(Event @event, ScriptAccessory accessory)
    {
    ...
    [ScriptMethod(name: "BOSS1 双头怪鸟_双头恐惧（直线）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^50658"])]
    public void 普通双头怪鸟_双头恐惧(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"双头恐惧{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
    ...
    [ScriptMethod(name: "BOSS1 绿头_冰雷簇（钢铁）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^5069[78]$"])]
    public void 普通绿头_冰雷簇(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"冰雷簇{@event.SourceId}";
        dp.Color = @event.ActionId() == 50697 ? new Vector4(1f, 0f, 1f, 1f) : new Vector4(0f, 1f, 1f, 1f);
        dp.Owner = @event.SourceId();
        dp.Scale = new Vector2(15f);
        dp.DestoryAt = 7700;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    ...
    [ScriptMethod(name: "BOSS1 绿头_冰焰凝环（月环安全区）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47660$"])]
    public void 普通绿头_冰焰凝环(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        var dp1 = accessory.Data.GetDefaultDrawProperties();
        ...
        dp.Radian = float.Pi * 2;
        dp1.DestoryAt = dp.DestoryAt = 2200;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Donut, dp);
    ...
    [ScriptMethod(name: "BOSS2 剑舞者_剑技风暴 AOE", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^49617$"])]
    public void 普通剑舞者_剑技风暴(Event @event, ScriptAccessory accessory)
    {
        // if (isText)accessory.Method.TextInfo($"AOE", duration: 4000, false);
        if (isTTS)accessory.Method.TTS($"AOE");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"AOE");
    ...
    [ScriptMethod(name: "BOSS2 剑舞者_秘法剑（半场刀）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^49585"])]
    public void 普通剑舞者_秘法剑(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"秘法剑{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor.WithW(1f);
        dp.Owner = @event.SourceId();
        dp.Scale = new (96f, 48f);
        dp.DestoryAt = 5200;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Rect, dp); 
    }
    
    [ScriptMethod(name: "BOSS2 剑舞者_跃进步法 剑技冲击（击退）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^49599$"])]
    public void 普通剑舞者_剑技冲击(Event @event, ScriptAccessory accessory)
    {   
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "剑舞者_剑技冲击击退预测";
        dp.Scale = new(1f, 24f);
        dp.Color = new Vector4(0f, 1f, 1f, 4f);
        dp.Owner = accessory.Data.Me;
        dp.TargetObject = @event.SourceId();
    ...
    [ScriptMethod(name: "BOSS2 剑舞者_舞动之剑 突进（直线）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^49616"])]
    public void 普通舞动之剑_突进(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"舞动之剑_突进{@event.SourceId}";
    ...
    [ScriptMethod(name: "BOSS3 惧死者_核爆雨（AOE）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47452$"])]
    public void 普通惧死者_核爆雨(Event @event, ScriptAccessory accessory)
    {
        if (isText)accessory.Method.TextInfo($"五连AOE", duration: 12700, true);
        if (isTTS)accessory.Method.TTS($"五连AOE");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"五连AOE");
    }
    
    [ScriptMethod(name: "BOSS3 惧死者_魔具联动：爆炎（钢铁）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^4746[58]$"])]
    public void 普通魔具联动_爆炎(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"魔具联动_爆炎{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
        dp.Scale = new Vector2(18f);
        dp.DestoryAt = 5200;
    ...
    [ScriptMethod(name: "BOSS3 惧死者_魔具联动：冰封（十字）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^4746[69]$"])]
    public void 普通魔具联动_冰封(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"魔具联动_冰封{@event.SourceId}";
        dp.Scale = new (15f, 90f);
        dp.Owner = @event.SourceId();
        dp.Color = accessory.Data.DefaultDangerColor.WithW(0.5f);
        dp.DestoryAt = 5200;
    ...
    [ScriptMethod(name: "BOSS3 屏障头_灭亡射线（小怪直线）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47475$"])]
    public void 普通屏障头_灭亡射线(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"屏障头_灭亡射线{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
        dp.Scale = new (6f, 30f);
        dp.DestoryAt = 4700;
    ...
    [ScriptMethod(name: "BOSS3 惧死者_魔具联动：暴雷（四向扇形）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47471$"])]
    public void 普通魔具联动_暴雷 (Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"魔具联动_暴雷{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
        dp.Scale = new Vector2(60f);
    ...
    [ScriptMethod(name: "BOSS4 目录_核爆（AOE）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^48415$"])]
    public void 普通目录_核爆(Event @event, ScriptAccessory accessory)
    {
        if (isText)accessory.Method.TextInfo($"大AOE", duration: 4000, true);
        if (isTTS)accessory.Method.TTS($"大AOE");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"大AOE");
    }
    
    [ScriptMethod(name: "BOSS4 目录_封印武器（弓）盯准 - 外围钢铁", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^48387$"])]
    public void 普通目录_盯准(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"目录_盯准{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
    ...
    [ScriptMethod(name: "BOSS4 目录_封印武器（琴）爱之歌 - 中间钢铁", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^48385$"])]
    public void 普通目录_爱之歌(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"目录_爱之歌{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = @event.SourceId();
        dp.Scale = new Vector2(15f);
        dp.DestoryAt = 6700;
        ...
        var dp1 = accessory.Data.GetDefaultDrawProperties();
        dp1.Name = $"目录_爱之歌描边{@event.SourceId}";
        dp1.Color = new Vector4(1f, 1f, 0f, 8f);
        dp1.Scale = new(15f);
        dp1.InnerScale = new(14.95f);
    ...
    [ScriptMethod(name: "BOSS4 目录_召唤（击杀小怪提示）", eventType: EventTypeEnum.AddCombatant, eventCondition: ["DataId:regex:^19296$"])]
    public void 目录_召唤(Event @event, ScriptAccessory accessory)
    {
        if (isText)accessory.Method.TextInfo($"击杀小怪", duration: 4000, true);
    ...
    public void 双头怪鸟_雷霜暴风雨(Event @event, ScriptAccessory accessory)
    {
        // if (isText)accessory.Method.TextInfo($"AOE", duration: 4000, false);
        if (isTTS)accessory.Method.TTS($"AOE");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"AOE");
    ...
    [ScriptMethod(name: "BOSS3 惧死者_核爆雨（AOE）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47482$"])]
    public void 惧死者_核爆雨(Event @event, ScriptAccessory accessory)
    {
        if (isText)accessory.Method.TextInfo($"五连AOE", duration: 12700, true);
        if (isTTS)accessory.Method.TTS($"五连AOE");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"五连AOE");
    ...
    [ScriptMethod(name: "BOSS3 惧死者_魔具联动：爆炎（钢铁）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^4749[04]$"])]
    public void 魔具联动_爆炎(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"魔具联动_爆炎{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
    ...
    [ScriptMethod(name: "BOSS3 惧死者_魔具联动：暴雷（四向扇形）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(47497|50358)$"])]
    public void 魔具联动_暴雷 (Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"魔具联动_暴雷{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
    ...
    [ScriptMethod(name: "BOSS3 惧死者_魔具联动：冰封（十字）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^4749[15]$"])]
    public void 魔具联动_冰封(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"魔具联动_冰封{@event.SourceId}";
        dp.Scale = new (15f, 90f);
    ...
    [ScriptMethod(name: "BOSS3 惧死者_黑暗奔流（面前直线）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47500$"])]
    public void 惧死者_黑暗奔流(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"惧死者_黑暗奔流";
        dp.Color = accessory.Data.DefaultDangerColor.WithW(0.6f);
    ...
    [ScriptMethod(name: "BOSS3 屏障头_灭亡射线（小怪直线）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^47504$"])]
    public void 屏障头_灭亡射线(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"屏障头_灭亡射线{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor;
    ...
    [ScriptMethod(name: "BOSS4 目录_全知烈火 引导分散+死刑（TTS）", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^50472$"])]
    public void 目录_全知烈火(Event @event, ScriptAccessory accessory)
    {
        var isTank = accessory.Data.MyObject?.IsTank() ?? false;
        if (isTank)
        {
            if (isText)accessory.Method.TextInfo($"引导T进发光平台（注意仇恨）", duration: 5000, true);
            if (isTTS)accessory.Method.TTS($"引导平台死刑或出去");
            if (isEdgeTTS)accessory.Method.EdgeTTS($"引导平台死刑或出去");
        }
        else
        {
            if (isText)accessory.Method.TextInfo($"引导平台分散或近战出去", duration: 5000, true);
            if (isTTS)accessory.Method.TTS($"引导平台分散或出去");
            if (isEdgeTTS)accessory.Method.EdgeTTS($"引导平台分散或出去");
        }
    }
    
    [ScriptMethod(name: "BOSS4 目录_魔法剑·石化（步进地火）直线预览", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^48445$"])]
    public void 魔法剑石化预兆(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"魔法剑石化预兆{@event.SourceId}";
        dp.Color = accessory.Data.DefaultDangerColor.WithW(1f);
        dp.Owner = @event.SourceId();
        dp.Scale = new (12f, 40f);
        dp.DestoryAt = 5000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Rect, dp); 
    }
