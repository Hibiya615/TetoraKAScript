# 南雲鉄虎个人KodakkuAssist绘制脚本

**These are scripts for Kodakku by Tetora**

纯画着玩，更新不保证，质量不保证，只适配CN版

Just fun, only CN ver.

## How-to-install  /  使用指南
- Use this link / 在线库链接： 

```https://raw.githubusercontent.com/Hibiya615/TetoraKAScript/main/OnlineRepo.json```

- Or download the Local version in **Releases** / 或者下载您所需要的文件并添加至您的**本地仓库**


感谢 [Ciceros](https://github.com/AdmiralLvtzov) 、[Cyf](https://github.com/cyf5119) 、[VV](https://github.com/VeeverSW) 等人的鼎力相助

Thanks for [Ciceros](https://github.com/AdmiralLvtzov) 、[Cyf](https://github.com/cyf5119) 、[VV](https://github.com/VeeverSW) and more.

## Scripts List   /  支持副本:

( CN / EN / JP ) [Text Translator](https://strings.wakingsands.com/) 

**迷宫挑战 / Dungeons / ダンジョン**
- `LV50 神兵要塞帝国南方堡` `LV50 最终决战天幕魔导城` `LV50 领航明灯天狼星灯塔` `LV50 惨剧灵殿塔姆·塔拉墓园`
- `LV55 邪龙王座龙巢神殿`
- `LV60 草木庭园圣茉夏娜植物园`
- `LV70 鏖战红莲阿拉米格` `LV70 疯狂战舰无限回廊` `LV70 污染庭园圣茉夏娜植物园`
- `LV80 异界遗构希尔科斯孪晶塔`
- `LV100 遗忘行路雾之迹`

**讨伐歼灭战 / Trials / 討伐・討滅戦**

- `LV50 伊弗利特歼灭战` `LV50 迦楼罗歼灭战` `LV50 拉姆歼灭战` `LV50 希瓦歼灭战` `LV50 莫古力贤王歼灭战` `LV50 那布里亚勒斯讨伐战`
  `LV50 究极神兵破坏作战` `LV50 海德拉讨伐战` `LV50 死化奇美拉讨伐战` `LV50 奥丁歼灭战` 
  `LV50 大桥上的决斗` `LV50 无限城的死斗`
- `LV60 尼德霍格征龙战` `LV60 萨菲洛特歼灭战` `LV60 索菲娅歼灭战` `LV60 祖尔宛歼灭战`
- `LV63 须佐之男歼灭战` `LV67 吉祥天女歼灭战` `LV70 神龙歼灭战` `LV70 月读歼灭战` `LV70 白虎镇魂战` `LV70 朱雀镇魂战` `LV70 保镖歼灭战`
- `LV73 缇坦妮雅歼灭战` `LV80 哈迪斯歼灭战`
- `LV90 高贝扎歼灭战` `LV90 泽罗姆斯歼灭战` `LV100 永恒女王歼灭战`
- `LV100 永远之暗歼灭战` `LV100 恩欧歼灭战`

**讨伐歼殛战 / Trials-Extreme**

- `LV50 究极神兵假想作战` `LV50 迦楼罗歼殛战` `LV50 伊弗利特歼殛战` `LV50 莫古力贤王歼殛战` `LV50 利维亚桑歼殛战` 
- `LV60 俾斯麦歼殛战`

**大型任务 / Raids / レイド**

***Normal***

- LV50：巴哈姆特大迷宫 / Bahamute

  `T5N`

- LV60：亚历山大机神城 / Alexander

  `A3N` `A5N` `A7N` `A10N` 

- LV70：欧米茄时空狭缝 / Omega Quests

  `O4N` `O5N` `O11N`
- LV80：伊甸希望乐园 / Eden

  `E1N` `E2N` `E8N` `E9N` `E10N` `E12N`

- LV90：万魔殿 / Pandæmonium

  `P10N` `P11N`


**团队任务 / Alliance Raids / アライアンスレイド**

- LV100: `温达斯：第三巡行`


**深层迷宫 / Deep Dungeons / ディープダンジョン**

- `死者宫殿 / 死者の宮殿 / The Palace of the Dead` 

- `朝圣交错路 / ピルグリム・トラバース / Pilgrim's Traverse`

**临危受命 / Fate**

`LV80 激斗畏惧装甲之秘密武器` `LV80 灾厄的古塔尼亚之深海讨伐战`

`LV90 兽道诸神信仰：伪神降临` `LV90 侵略兵器召回指令：破坏侵略兵器希`

`LV100 蛇王得酷热涅：荒野的死斗` `LV100 亩鼠米卡：盛装巡游皆大欢喜`

**特殊场景探索 / Field Operations / 特殊フィールド探索**

**天青斗场 / Blue Sky / ブルースカイ**

`18 - 爆破死斗`


### 【施工中 / Building / 工事中】

**请注意：** 测试仓库内的脚本均为**未实际验证** 或者**有未修复的BUG** 请慎用

**测试仓库链接**

> https://raw.githubusercontent.com/Hibiya615/TetoraKAScript/main/TestScriptRepo.json

`正统优雷卡`

`禁地优雷卡 - 巴尔德西昂兵武塔`