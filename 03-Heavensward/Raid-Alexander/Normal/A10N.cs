...
public class NewDuty
{
    ...
    [ScriptMethod(name: "机制销毁", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:regex:^(685[459]|6860)$"],userControl: false)]
    public void 机制销毁(Event @event, ScriptAccessory accessory)
    {
        switch (@event.ActionId())
        {
            case 6854:
                accessory.Method.RemoveDraw($"锤击");
                break;
            case 6855:
                accessory.Method.RemoveDraw($"寒冰箭");
                break;
            case 6859:
                accessory.Method.RemoveDraw($"哥布流乱斩"); // 单发填充月环
                break;
            case 6860:
                accessory.Method.RemoveDraw($"哥布流回旋斩"); // 单发填充钢铁
                break;

        }
    }
    
    [ScriptMethod(name: "尖刺陷阱销毁", eventType: EventTypeEnum.ObjectChanged, eventCondition: ["DataId:regex:^80[89]$"],userControl: false)]
    public void 尖刺陷阱销毁(Event @event, ScriptAccessory accessory)
    {
         accessory.Method.RemoveDraw($"尖刺陷阱");
    }
    
    #endregion
    
    [ScriptMethod(name: "尖刺陷阱高亮", eventType: EventTypeEnum.ChangeMap, eventCondition: ["MapId:regex:^329$"])]
    public void 尖刺陷阱高亮(Event @event, ScriptAccessory accessory)
    {
    ...
    [ScriptMethod(name: "冰晶陷阱_寒冰箭（左右边）", eventType: EventTypeEnum.ObjectEffect, eventCondition: ["Id1:65", "Id2:32"])]
    public void 寒冰箭(Event @event, ScriptAccessory accessory)
    {
        if (isTTS)accessory.Method.TTS($"冰晶陷阱");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"冰晶陷阱");
    ...
    [ScriptMethod(name: "铁球陷阱_锤击（圆形）", eventType: EventTypeEnum.ObjectEffect, eventCondition: ["Id1:1", "Id2:32"])]
    public void 锤击(Event @event, ScriptAccessory accessory)
    {
        if (isTTS)accessory.Method.TTS($"铁球陷阱");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"铁球陷阱");
        ...
        // 铁球与旋转链锯的 DataId均为6151，所以采用固定坐标绘制
    ...
    [ScriptMethod(name: "必中决心（红标分散）", eventType: EventTypeEnum.TargetIcon, eventCondition: ["Id:regex:^0019$"])]
    public void 必中决心(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"必中决心{@event.SourceId}";
    ...
    [ScriptMethod(name: "哥布流哥布林冲锋 死刑提示", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^6863$"])]
    public void 哥布流哥布林冲锋(Event @event, ScriptAccessory accessory)
    {
        var isTank = accessory.Data.MyObject?.IsTank() ?? false;
        if (isTank && isText)accessory.Method.TextInfo($"坦克死刑", duration: 4000, true);
        if (isTTS)accessory.Method.TTS($"坦克死刑");
        if (isEdgeTTS)accessory.Method.EdgeTTS($"坦克死刑");
    }
    
    [ScriptMethod(name: "单发填充 - 哥布流回旋斩（钢铁）", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:regex:^6841$"])]
    public void 哥布流回旋斩(Event @event, ScriptAccessory accessory)
    {
    ...
    [ScriptMethod(name: "单发填充 - 哥布流乱斩（月环）", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:regex:^6840$"])]
    public void 哥布流乱斩(Event @event, ScriptAccessory accessory)
    {
    ...
    [ScriptMethod(name: "旋转链锯_斩击", eventType: EventTypeEnum.Chat, eventCondition: ["Type:NPCDialogueAnnouncements", 
    ...
    public void 斩击(Event @event, ScriptAccessory accessory)
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"斩击";
        dp.Color = accessory.Data.DefaultDangerColor.WithW(0.5f);
        ...
        /* 备用
        foreach (var item in accessory.Data.Objects.GetByDataId(6151))
        {
            var dp = accessory.Data.GetDefaultDrawProperties();
            dp.Name = $"斩击";
            dp.Color = accessory.Data.DefaultDangerColor;
